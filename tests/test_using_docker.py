...
def test_job_test_docker_exists(jenkins):
    jenkins.assert_job_exists("test-docker")


def test_job_test_docker_build_created(jenkins):
    jenkins.start_build("test-docker")


def test_job_test_docker_suceeded(jenkins):
    jenkins.wait_for_build_to_finish("test-docker")
    jenkins.assert_build_succeeded("test-docker")
