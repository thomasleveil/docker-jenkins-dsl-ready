...
def test_job_test_docker_exists(jenkins):
    r = jenkins.get("/job/test-docker/")
    assert 200 == r.status_code


def test_job_test_docker_build_created(jenkins):
    r = jenkins.post('/job/test-docker/build')
    assert 201 == r.status_code, "Failed to start a build"
    wait_until(timeout=60.0, pause=0.5,
               check=lambda: jenkins.get("/job/test-docker/1/").status_code == 200)


def test_job_test_docker_suceeded(jenkins):
    wait_until(timeout=60.0, pause=0.5,
               check=lambda: job_not_building(jenkins, "test-docker"))

    r = jenkins.get("/job/test-docker/1/api/json")
    assert 200 == r.status_code
    assert 'SUCCESS' == r.json()['result']
